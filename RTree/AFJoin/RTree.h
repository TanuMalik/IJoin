...
public:
  ...
  typedef int (*t_resultCallback)(DATATYPE, void*);
...
protected:
  ...
  int Search(Node* a_node, Rect* a_rect, int& a_foundCount, t_resultCallback a_resultCallback, void* a_context);
...
// False == -1
// True == 0
// Modified from bool to int for returning matched id 
RTREE_TEMPLATE
int RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, t_resultCallback a_resultCallback, void* a_context)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(Search(a_node->m_branch[index].m_child, a_rect, a_foundCount, a_resultCallback, a_context) < 0)
        {
  ...
  else
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        ...
        if(a_resultCallback)
        {
	  printf("Hit first time %d",global++);
	  //TM comment
          //if(!a_resultCallback(id, a_context) >= 0)
          if(a_resultCallback(id, a_context) >= 0)
          {
            return id; // Don't continue searching
          }
        }
      }
    }
